...
export default function SeriesCard({ content }: SeriesCardProps) {
  const title = content.name || 'Unknown Title';
  const firstAirDate = content.first_air_date;
  const posterPath = content.poster_path;

  return (
    <Link href={`/non-translated/series/${content.id}`} className="group block">
